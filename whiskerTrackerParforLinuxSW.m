...
% Alt version for single whisker 
...
parfor n=1:length(measures)
    ...
    system(['/home/hireslab/Code/whisk-1.1.0d-Linux/bin/whisk/measure ' '--face ' 'left ' measures(n).name ' ' outputFileName '.measurements']);
...
% Use for multiple whiskers, see below for single whiskers 
% classes = dir('*.measurements');
% 
% parfor n=1:length(classes)
%     [~, outputFileName] = fileparts(classes(n).name);
%     system(['/home/hireslab/Code/whisk-1.1.0d-Linux/bin/whisk/classify ' classes(n).name ' ' outputFileName '.measurements ' 'left ' '--px2mm ' trackingInfo(3) '-n ' '1']);
...
parfor n=1:length(classes)
    [~, outputFileName] = fileparts(classes(n).name);
    system(['/home/hireslab/Code/whisk-1.1.0d-Linux/bin/whisk/classify-single-whisker ' classes(n).name ' ' outputFileName '.measurements']);
    display([classes(n).name ' has been classified'])
end
...
parfor n=1:length(classes)
    [~, outputFileName] = fileparts(classes(n).name);
    system(['/home/hireslab/Code/whisk-1.1.0d-Linux/bin/whisk/reclassify ' classes(n).name ' ' outputFileName '.measurements' ' ' '-n' '1']);
